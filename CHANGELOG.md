...
### Changed
- **Artifact renames (2026-08-10) — all open naming targets closed:** `OEMInsightBI_v2` → **`OEMInsightBI`** (dropped the version suffix), `sample-quality-data` → **`sample_quality_data`** (last hyphenated notebook name), and `report2` → **`oem_report`** (co-named with its semantic model). Each done as a live-rename-first lockstep: workspace `displayName` first, then repo `git mv` + `.platform`, the `definition.pbir` model-binding path, and the `test_doc_artifact_names` guard. Item IDs are unchanged by a rename, so the report→model binding was never disturbed.
