...
- **Python is uv-managed — run everything through `uv run`.** `.python-version` pins 3.13 and `pyproject.toml` pins `pyspark==4.0.1`; `uv run pytest tests/` provisions the interpreter and deps and runs 300 tests in ~30s. **A bare `python3` is not the project interpreter** and may be an unrelated system Python — never assume it. `pyproject.toml` is deliberately non-packaging (`tool.uv.package = false`): `tests/` imports `from src.transformations…` via the repo root on `sys.path`, and installing `src/` would make tests import the installed copy instead of the working tree, silently voiding the src/-vs-notebook parity contract. CI runs the same lock across 3.10–3.13.
- **PySpark transformation logic IS locally testable.** A `local[1]` SparkSession over an in-memory fixture runs pure-DataFrame logic in under a minute. Do not accept "no Spark session, Fabric-only" as a verification ceiling for transformation code — execute it. The genuine local ceiling is **Delta-specific operations** (MERGE, time travel, **write-time column-name validation** — `delta-spark` is *not* installed) and anything lakehouse-dependent. Established 2026-07-23 when verify-agent measured task-027's restructure (9/9 assertions) that both the task notes and the dispatch brief had assumed could only be reasoned about. Delta rejects column names containing ` ,;{}()\n\t=` (e.g. a space) unless Column Mapping is enabled — a bronze write of a CSV with a spaced header (Yale's `EPI Percent`) crashes at `saveAsTable` with `DELTA_INVALID_CHARACTERS_IN_COLUMN_NAMES`, and local Spark never invokes a Delta writer so the failure is invisible until the Fabric pipeline runs. The danger direction is the same as ANSI: local passes where Fabric fails. Surfaced 2026-08-04 when task-056's bronze→gold run failed after a 7/7 local pass; fix was a snake_case rename (`EPI Percent` → `epi_percent`) at write time, after the source-header validation gate (commit 626a21c).
