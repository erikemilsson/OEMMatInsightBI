...
| `data_type_consistency` | `silver_procurement` | `75.00` → **`100.00`** | `fail` → **`pass`** | `false` | **Acceptance revisited 2026-08-12 (DEC-016)** — the silver layer was fixed, not the expectation; see 7.3. Confirmed live on two runs (15:28:48Z full-load, 16:06:16Z incremental) |
...
1. **`unitpriceeur` carries float32-image rounding noise.** 124 of 132 values had more than two decimal places — e.g. `17.309999465942383` for an intended `17.31`, `1.350000023841858` for `1.35`. The magnitudes are exactly what a `float32` value widened to `float64` produces. Because `spend_eur = quantity × unitpriceeur`, that noise propagated into gold and into `Total Spend EUR`. At 132 rows the absolute error is sub-cent, but storing money as a float is a genuine anti-pattern rather than a cosmetic one.
   ...
   > ✅ **RE-MEASURED 2026-08-12: now `0` of 132.** The "124 of 132" above is the 2026-08-06 measurement the acceptance decision was made on, preserved because deleting it would erase that evidence — **but it is no longer current, so do not cite it as such.** The 2026-08-12 re-seed from `azure/procurement_seed.sql` wrote clean 2-decimal literals, and the task-069 rebuild then landed them in a `decimal(18,2)` column. Measured live after the rebuild:
   > `SELECT COUNT(*) FROM oem_lh.bronze_procurement_transactional WHERE UnitPriceEUR <> ROUND(UnitPriceEUR, 2)` → **0**. (Closes FR-088.)
   > Corroborating: `SUM(spend_eur)` over `fact_procurement` reads **3273776.03** on 132 rows — *identical* to the task-024 baseline. That identity is the expected result, not a coincidence: the values were already exact to the cent, so the change removed only sub-cent representation error, which rounds away. Precision improved; semantics did not move.
...
**Consequences — ALL CONFIRMED LIVE 2026-08-12.** These were written as expectations before the run and are left in place with their measured outcomes, so the record shows what was predicted versus what happened. Local verification was structurally incapable of settling any of them: `delta-spark` is not installed and local Spark never invokes a Delta writer, so `DELTA_FAILED_TO_MERGE_FIELDS` cannot be reproduced locally. Same hazard family as `DELTA_INVALID_CHARACTERS_IN_COLUMN_NAMES`.

| # | Expectation | Measured outcome |
|---|---|---|
| 1 | `data_type_consistency` moves **75.00 → 100.00** — and for migration step 5's reason: the layer implements the declared intent, rather than the expectation being bent to fit | ✅ **100.00 / `pass`** on both the full-load run (15:28:48Z) and the post-rebuild incremental (16:06:16Z). The expectation dict is byte-unchanged — the layer moved to meet it |
| 2 | Bronze lands `decimal(18,2)` after the repo DDL + seed are re-run | ✅ `Quantity: DecimalType(18,2)`, `UnitPriceEUR: DecimalType(18,2)` — read from the live schema after a real Copy run, not inferred |
| 3 | The incremental append survives the first *scheduled* run after the full-load rewrite | ✅ Normal `p_full_load = false` run green; `bronze_load_metadata` shows `rows_loaded = 11` `SUCCESS` (the 7-day lookback from the `2024-12-31` watermark), no `FAILED` row. The 11 is what proves the append path ran rather than a silent full load |
| 4 | `fact_procurement.unitprice_eur` and `spend_eur` are still `double`; the TMDL needs no edit | ✅ `quantity_base`, `unitprice_eur`, `spend_eur` all `DoubleType`. TMDL untouched. `gold_low_confidence_audit.spend_impact` also still `double` (FR-089 — safe by union coercion, not by a pin) |
| 5 | `Total Spend EUR` moves **only by recovered precision** against the task-024 baseline `sum_spend_eur = 3273776.03` | ✅ **3273776.03** on 132 rows — *identical*. That is the correct outcome: the values were already exact to the cent, so only sub-cent representation error was removed and it rounds away. `oem_report` renders 3.27M and re-framed against the rewritten Delta files without issue |
| 6 | The >2dp count on `bronze.UnitPriceEUR` is **0 of 132** (was 124) | ✅ **0**. Settles the stale figure in sub-finding 1 above and closes FR-088 |
