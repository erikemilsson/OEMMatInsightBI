...
**Two checks record a non-`pass` row on every single run, by design.** If you meet `dq_business_rule_validation = 81.82` (`silver_procurement`, `status = 'fail'`) or `dq_date_range_validation = 0.0` (`bronze_procurement_transactional`, `status = 'warning'`), they are measured, explained and accepted. A third, `dq_data_type_consistency = 75.0`, did so until task-069/DEC-016 fixed the silver-boundary cast on 2026-08-12; it now reads `100.0 / pass`, and the 75.0 value survives only in history rows predating that fix — not unnoticed defects, and none is in `BLOCKING_CHECKS`, so the gate correctly never raises on them. Full diagnosis with live evidence, the reporting consequences, and a migration path: **`data_quality_framework.md` § 7 "Persistent Advisory Failures (Measured and Accepted)"**.
